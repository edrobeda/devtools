...
// Caracteres considerados "invisíveis" ou que costumam causar surpresas.
const INVISIBLE = new Set([
  ...
  '\u061C', // arabic letter mark
  '\u115F', '\u1160', // hangul choseong filler / jungseong filler
  '\u17B4', '\u17B5', // khmer invisible marks
  '\u180B', '\u180C', '\u180D', '\u180E', // mongolian free variation selectors / vowel separator
  '\u2000', '\u2001', '\u2002', '\u2003', '\u2004', '\u2005', '\u2006', '\u2007',
  '\u2008', '\u2009', '\u200A', '\u2028', '\u2029', '\u202F', '\u205F',
...
function blockName(cp) {
  for (const [start, end, name] of BLOCK_RANGES) {
...
function categoryOf(char) {
  ...
  // ASCII control
  if (cp < 0x20 || cp === 0x7F || (cp >= 0x80 && cp <= 0x9F)) return 'C'
...
function codePointName(cp) {
  if (NAMED_CONTROLS[cp]) return NAMED_CONTROLS[cp]
  return `U+${cp.toString(16).toUpperCase().padStart(4, '0')} (${blockName(cp)})`
}

function htmlEntity(char) {
  const cp = char.codePointAt(0)
  if (cp > 127 || [34, 38, 60, 62].includes(cp)) return `&#${cp};`
  return char
}
...
export function countGraphemeClusters(text) {
...
export function listCodePoints(text) {
  ...
  for (const char of chars) {
    if (!seen.has(char)) {
      seen.set(char, { char, count: 0, firstIndex: order.length })
      order.push(char)
    }
    seen.get(char).count += 1
  }
...
export function toEscaped(text, mode) {
  const chars = Array.from(text)
  switch (mode) {
    case 'js':
      return chars
        .map((c) => {
          ...
          if (cp > 127) return `\\u${cp.toString(16).toUpperCase().padStart(4, '0')}`
          return c
        })
        .join('')
    case 'python':
      return chars
        .map((c) => {
          ...
          if (cp === 0x09) return '\\t'
          if (cp < 0x20 || cp === 0x7f) return `\\x${cp.toString(16).padStart(2, '0')}`
          if (cp > 0xffff) return `\\U${cp.toString(16).toUpperCase().padStart(8, '0')}`
          if (cp > 127) return `\\u${cp.toString(16).toUpperCase().padStart(4, '0')}`
          return c
        })
        .join('')
    case 'html':
      return chars.map((c) => htmlEntity(c)).join('')
    case 'css':
      return chars
        .map((c) => {
          const cp = c.codePointAt(0)
          if (cp > 127 || cp === 0x22 || cp === 0x5c) return `\\${cp.toString(16).toUpperCase()} `
          return c
        })
        .join('')
    default:
...
export const ESCAPE_MODES = ['js', 'python', 'html', 'css']

export function normalize(text, form) {
  if (!NORMALIZATION_FORMS.includes(form)) throw new Error(`Unknown normalization form: ${form}`)
