...
const ROUTES = ['/', '/references/python-cheatsheet']
...
async function main() {
  ...
  for (const route of ROUTES) {
    ...
    if (route.includes('python-cheatsheet') && info.listItems < 10) failed++
    await page.close()
  }
  ...
  await page.goto('https://devtools.eventifylab.com/references/python-cheatsheet', { waitUntil: 'networkidle0' })
  ...
  // search filter
  await page.type('.ant-input-affix-wrapper input, input.ant-input', 'comprehension', { delay: 10 })
  await new Promise((r) => setTimeout(r, 300))
  const itemsSearch = await page.evaluate(() => document.querySelectorAll('.ant-list-item').length)
  console.log('items("comprehension"):', itemsSearch)
  ...
  await page.evaluate(() => {
    ...
    const arr = labels.find((el) => el.innerText.startsWith('Listas'))
    if (arr) arr.click()
  })
  await new Promise((r) => setTimeout(r, 300))
  const itemsCat = await page.evaluate(() => document.querySelectorAll('.ant-list-item').length)
  console.log('items(Listas):', itemsCat)
  ...
  console.log('EN contains "Python Cheat Sheet":', enText.includes('Python Cheat Sheet'))
