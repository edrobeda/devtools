...
const ROUTES = ['/', '/references/javascript-cheatsheet']
...
async function main() {
  ...
  for (const route of ROUTES) {
    ...
    if (route.includes('javascript-cheatsheet') && info.listItems < 10) failed++
    await page.close()
  }

  // exercise the new page: search, category filter, copy buttons, EN switch
  const page = await browser.newPage()
  const pgErrs = []
  page.on('pageerror', (e) => pgErrs.push('pageerror: ' + e.message))
  page.on('console', (m) => { if (m.type() === 'error') pgErrs.push('console: ' + m.text()) })
  await page.goto('https://devtools.eventifylab.com/references/javascript-cheatsheet', { waitUntil: 'networkidle0' })
  ...
  // search filter
  await page.type('.ant-input-affix-wrapper input, input.ant-input', 'reduce', { delay: 10 })
  await new Promise((r) => setTimeout(r, 300))
  const itemsSearch = await page.evaluate(() => document.querySelectorAll('.ant-list-item').length)
  console.log('items("reduce"):', itemsSearch)
  ...
  await page.evaluate(() => {
    ...
    const arr = labels.find((el) => el.innerText.startsWith('Arrays'))
    if (arr) arr.click()
  })
  await new Promise((r) => setTimeout(r, 300))
  const itemsCat = await page.evaluate(() => document.querySelectorAll('.ant-list-item').length)
  console.log('items(Arrays):', itemsCat)
  if (itemsCat === 0 || itemsCat === itemsBefore) failed++

  // copy a single snippet via its row button
  await page.evaluate(() => {
  ...
  // copy as markdown (may fail on clipboard perms — should still not crash)
  await page.evaluate(() => {
  ...
  console.log('EN contains "Cheat Sheet":', enText.includes('Cheat Sheet') || enText.includes('JavaScript'))
