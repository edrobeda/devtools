...
const ROUTES = ['/', '/references/grep-sed-awk']
...
async function main() {
  ...
  for (const route of ROUTES) {
    ...
    if (route.includes('grep-sed-awk') && info.listItems < 40) failed++
    await page.close()
  }
  ...
  if (itemsBefore < 40) failed++

  // search filter (matches sed -n content)
  await page.type('.ant-input-affix-wrapper input, input.ant-input', 'awk {print', { delay: 5 })
  await new Promise((r) => setTimeout(r, 300))
  const itemsSearch = await page.evaluate(() => document.querySelectorAll('.ant-list-item').length)
  console.log('items("awk {print"):', itemsSearch)
  ...
  await page.evaluate(() => {
    ...
    const arr = labels.find((el) => el.innerText.startsWith('find'))
    if (arr) arr.click()
  })
  await new Promise((r) => setTimeout(r, 300))
  const itemsCat = await page.evaluate(() => document.querySelectorAll('.ant-list-item').length)
  console.log('items(find & xargs):', itemsCat)
  if (itemsCat === 0 || itemsCat === itemsBefore) failed++

  // back to all, test copy single button (won't crash on clipboard perms)
  await page.evaluate(() => {
    const labels = Array.from(document.querySelectorAll('.ant-radio-button-wrapper'))
  ...
  console.log('EN contains "grep/sed/awk Cheat Sheet":', enText.includes('grep/sed/awk Cheat Sheet'))
  if (!enText.includes('grep/sed/awk Cheat Sheet')) failed++
  ...
  console.log('menu has grep/sed/awk:', ptText.includes('grep/sed/awk'))
  if (!ptText || !ptText.includes('grep/sed/awk')) failed++
